...
let array = []
    ...
    inp.addEventListener("input", function(e) {
        ...
        var pos = arr[i].name.toUpperCase().indexOf(val.toUpperCase());
          /*check if the item starts with the same letters as the text field value:*/
          if (pos > -1) {
            if(a.childNodes.length >= 0){
            document.querySelector('.plus-icon').classList.add('d-none')
           }
            /*create a DIV element for each matching element:*/
            b = document.createElement("DIV");
            
            /*make the matching letters bold:*/
            b.innerHTML = arr[i].name.substr(0, pos);
            b.innerHTML += "<strong>" + arr[i].name.substr(pos, val.length) + "</strong>";
            b.innerHTML += arr[i].name.substr(pos + val.length);
            ...
            document.querySelector('.plus-icon').classList.remove('d-none')
        }
    }
});
...
function callapi(){
    let fetchRes = fetch(
        ...
        fetchRes.then(res =>
            res.json()).then(d => {
            console.log(d.data)
            autocomplete(document.getElementById("id_property_location"), d.data);
        })
}
